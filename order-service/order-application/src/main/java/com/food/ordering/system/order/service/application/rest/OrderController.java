...
public class    OrderController {
    ...
    public ResponseEntity<CreateOrderResponse> createOrder(@RequestBody CreateOrderCommand createOrderCommand) {
        log.info("Received request to create order for customer: {} at restaurant: {}",
    ...
    public ResponseEntity<TrackOrderResponse> getOrderByTrackingId(@PathVariable UUID trackingId) {
        log.info("Received request to get order for trackingId: {}", trackingId);
        TrackOrderResponse trackOrderResponse =
                orderApplicationService.trackOrder(TrackOrderQuery.builder().orderTrackingId(trackingId).build());
        log.info("Order tracked with trackingId: {}", trackOrderResponse.getOrderTrackingId());
